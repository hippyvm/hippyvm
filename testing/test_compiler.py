...
class TestCompiler(object):
    ...
    def test_function_decl(self):
        ...
        DECLARE_FUNC 0
        """)
        assert bc.user_functions[0].types == [consts.ARG_ARGUMENT,
    ...
    def test_function_decl_2(self):
        ...
        DECLARE_FUNC 0
        """)
        assert bc.user_functions[0].types == []
        self.compare(bc.user_functions[0].bytecode, """
        LOAD_NULL
        RETURN
        LOAD_NULL   # unreachable
    ...
    def test_initialized_static(self):
        ...
        assert isinstance(bc.consts[0].deref_temp(), floatobject.W_FloatObject)
    ...
    def test_class_decl(self):
        ...
        DECLARE_CLASS 1
        """)
        assert isinstance(bc.user_functions[0], klass.UserClass)
        assert bc.user_functions[0].name == 'Aa'
        assert bc.user_functions[1].name == 'bB'
    ...
    def _class_decl(self, source):
        ...
        """)
        assert isinstance(bc.user_functions[0], klass.UserClass)
        return bc.user_functions[0]

    def test_class_property(self):
        klass = self._class_decl('class A { public $foo; };')
        assert 'foo' in klass.properties
        pfoo = klass.properties['foo']
    ...
    def test_class_property_several(self):
        ...
        assert 'foo' in klass.properties
        assert 'bar' in klass.properties
        assert 'baz' in klass.properties
        space = self.space
        assert space.is_w(klass.properties['foo'].value, space.w_Null)
        assert space.is_w(klass.properties['bar'].value, space.newint(2))
        assert space.is_w(klass.properties['baz'].value, space.w_Null)

    def test_class_extends(self):
        klass = self._class_decl("class A extends B { }")
    ...
    def test_class_method(self):
        ...
        assert 'foo' in klass.methods
        mfoo = klass.methods['foo']
        assert mfoo.access_flags == consts.ACC_PUBLIC
        func = mfoo.method_func
    ...
    def test_abstract_method(self):
        ...
        assert klass.methods['f'].access_flags & consts.ACC_ABSTRACT
