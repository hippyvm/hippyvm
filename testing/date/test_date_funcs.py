...
class TestDateFuncs(BaseTestInterpreter):
    ...
    def test_date_parse(self):
        ...
        output = self.run("""
            echo date_parse("2006-12-12 10:00:00.5");
        """)

        val = output.pop(0).as_rdict()

        assert self.space.int_w(val['year']) == 2006
        assert self.space.int_w(val['month']) == 12
        assert self.space.int_w(val['day']) == 12
        assert self.space.int_w(val['hour']) == 10
        assert self.space.int_w(val['minute']) == 0
        assert self.space.int_w(val['second']) == 0
        assert self.space.int_w(val['fraction']) == 0
        assert self.space.int_w(val['warning_count']) == 0
        assert self.space.int_w(val['error_count']) == 0
        assert val['is_localtime'] == self.space.w_False

    def test_date_parse_from_format(self):

        output = self.run("""
            $date = "6.1.2009 13:00+01:00";
            echo date_parse_from_format("j.n.Y H:iP", $date);
        """)

        val = output.pop(0).as_rdict()

        assert self.space.int_w(val['year']) == 2009
        assert self.space.int_w(val['month']) == 1
        assert self.space.int_w(val['day']) == 6
        assert self.space.int_w(val['hour']) == 13
        assert self.space.int_w(val['minute']) == 0
        assert self.space.int_w(val['second']) == 0
        assert self.space.int_w(val['fraction']) == 0
        assert self.space.int_w(val['warning_count']) == 0
        assert self.space.int_w(val['error_count']) == 0
        assert val['is_localtime'] == self.space.w_True

    def test_timezone_location_get(self):
        ...
        assert self.space.str_w(val['country_code']) == "CZ"
        assert abs(self.space.float_w(val['latitude']) - 50.08333) < 0.0001
        assert abs(self.space.float_w(val['longitude']) - 14.43333) < 0.0001
        assert self.space.str_w(val['comments']) == ''
