...
/* {{{ 3HAVALTransform
 */
static void HAVAL3_Transform(uint32_t state[8], const unsigned char block[128])
{
	uint32_t E[8];
	uint32_t x[32];
	int i;
...
/* {{{ 4HAVALTransform
 */
static void HAVAL4_Transform(uint32_t state[8], const unsigned char block[128])
{
	uint32_t E[8];
	uint32_t x[32];
	int i;
...
/* {{{ 5HAVALTransform
 */
static void HAVAL5_Transform(uint32_t state[8], const unsigned char block[128])
{
	uint32_t E[8];
	uint32_t x[32];
	int i;
...
#define HASH_HAVAL_INIT(p,b) \
void HAVAL##p##_##b##Init(HAVAL_CTX *context) \
{	int i; context->count[0] = 	context->count[1] = 	0; \
	for(i = 0; i < 8; i++) context->state[i] = D0[i]; \
	context->passes = p;	context->output = b; \
	context->Transform = HAVAL##p##_Transform; }


HASH_HAVAL_INIT(3,128)
HASH_HAVAL_INIT(3,160)
HASH_HAVAL_INIT(3,192)
HASH_HAVAL_INIT(3,224)
HASH_HAVAL_INIT(3,256)

HASH_HAVAL_INIT(4,128)
HASH_HAVAL_INIT(4,160)
HASH_HAVAL_INIT(4,192)
HASH_HAVAL_INIT(4,224)
HASH_HAVAL_INIT(4,256)

HASH_HAVAL_INIT(5,128)
HASH_HAVAL_INIT(5,160)
HASH_HAVAL_INIT(5,192)
HASH_HAVAL_INIT(5,224)
HASH_HAVAL_INIT(5,256)

/* {{{ HAVALUpdate
 */
void HAVALUpdate(HAVAL_CTX *context, const unsigned char *input, unsigned int inputLen)
{
...
#define HASH_HAVAL_VERSION	0x01

/* {{{ HAVAL128Final
 */
void HAVAL128Final(unsigned char *digest, HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;

	/* Version, Passes, and Digest Length */
	bits[0] =	(HASH_HAVAL_VERSION & 0x07) |
	...
	HAVALUpdate(context, PADDING, padLen);

	/* Append version, passes, digest length, and message length */
	HAVALUpdate(context, bits, 10);
...
/* {{{ HAVAL160Final
 */
void HAVAL160Final(unsigned char *digest, HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
...
/* {{{ HAVAL192Final
 */
void HAVAL192Final(unsigned char *digest, HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
...
/* {{{ HAVAL224Final
 */
void HAVAL224Final(unsigned char *digest, HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
...
/* {{{ HAVAL256Final
 */
void HAVAL256Final(unsigned char *digest, HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
