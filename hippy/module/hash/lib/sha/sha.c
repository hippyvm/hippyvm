...
/* {{{ SHA224Update
   ...
   context.
 */
void SHA224Update(SHA224_CTX * context, const unsigned char *input, unsigned int inputLen)
{
	unsigned int i, index, partLen;
...
/* {{{ SHA224Final
   ...
   the message digest and zeroizing the context.
 */
void SHA224Final(unsigned char digest[28], SHA224_CTX * context)
{
	unsigned char bits[8];
	unsigned int index, padLen;
	...
	index = (unsigned int) ((context->count[0] >> 3) & 0x3f);
	padLen = (index < 56) ? (56 - index) : (120 - index);
	SHA224Update(context, PADDING, padLen);
...
/* {{{ SHA384Update
   ...
   context.
 */
void SHA384Update(SHA384_CTX * context, const unsigned char *input, unsigned int inputLen)
{
	unsigned int i, index, partLen;
...
/* {{{ SHA384Final
   ...
   the message digest and zeroizing the context.
 */
void SHA384Final(unsigned char digest[48], SHA384_CTX * context)
{
	...
	SHA384Update(context, PADDING, padLen);
