...
class ArrayMultisortFunction(BuiltinFunction):
    name = identifier = _fullname = 'array_multisort'
    runner = None
...
def _count(space, w_arr, recursive, references=None):
    if not references:
        references = {}

    if not isinstance(w_arr, W_ArrayObject):
        return int(w_arr is not space.w_Null)
            ...
            while not w_iter.done():
                ...
                if isinstance(w_val, W_ArrayObject):
                    ...
                    if references[w_val] > 1:
                        ...
                        return _len
                    _len = _len + _count(space, w_val, recursive, references)

        return _len
    return space.arraylen(w_arr)
...
def count(interp, w_arr, recursive=0):
    ...
    return interp.space.wrap(_count(interp.space, w_arr, recursive))
...
@wrap(['space', ArrayArg(None), Optional(W_Root), Optional(str)])
def extract(space, w_arr, w_extr=None, prefix=None):
...
@wrap(['space', 'reference'])
def natsort(space, w_ref):
    """ Sort an array using a "natural order" algorithm """
    # from hippy.string_funcs import _strnatcmp
    w_arr = w_ref.deref()
    if w_arr.tp != space.tp_array:
        space.ec.warn("natsort() expects parameter 1 "
                      "to be array, %s given" %
                      space.get_type_name(w_arr.tp))
        return space.w_False
    pairs = w_arr.as_pair_list(space)
    w_natsort_func = space.ec.interpreter.functions['strnatcmp']
...
@wrap(['space', 'reference'])
def natcasesort(space, w_ref):
    """ Sort an array using a "natural order" algorithm """
    # from hippy.string_funcs import _strnatcasecmp
    w_arr = w_ref.deref()
    if w_arr.tp != space.tp_array:
        space.ec.warn("natcasesort() expects parameter 1 "
                      "to be array, %s given" %
                      space.get_type_name(w_arr.tp))
        return space.w_Null
    pairs = w_arr.as_pair_list(space)
    w_natsort_func = space.ec.interpreter.functions['strnatcasecmp']
    _sort(space, pairs, sort_type=2, elem=VALUE, cmp=w_natsort_func)
    w_ref.store(space.new_array_from_pairs(pairs))
    return space.w_True
