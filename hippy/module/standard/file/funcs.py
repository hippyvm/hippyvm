...
def file_get_contents(space, fname, num_args, use_include_path=False,
    ...
    if not is_in_basedir(space, 'file_get_contents', fname):
        ...
        return space.w_False
    if not _valid_fname(fname):
        space.ec.warn("file_get_contents() expects parameter 1 "
                      "to be a valid path, string given")
        return space.w_Null
    ...
    try:
        ...
        space.ec.warn("file_get_contents(%s): failed to "
...
def _fopen(space, fname, mode, w_use_include_path=False, w_ctx=None):
    ...
    orig_fname = fname
    # XXX implement parsing URL, from wherever we have it implemented
    if fname.startswith('php://'):
        if fname == 'php://':
    ...
    if fname.startswith('file://'):
        ...
        raise FopenError(
                ["remote host file access not supported, %s" % fname,
    ...
    try:
        w_res = W_FileResource(space, fname, mode)
        w_res.open()
        return w_res
    except IOError, e:
        if not we_are_translated():
...
def fopen(space, fname, mode, w_use_include_path=False, w_ctx=None):
    ...
    if not is_in_basedir(space, 'fopen', fname):
        space.ec.warn("fopen(%s): failed to open stream: Operation not permitted" % fname)
        return space.w_False
...
def popen(space, command, mode):
    ...
    raise Exception("popen not RPython")
    try:
        a = os.popen(command, mode)
...
def readfile(space, fname, use_include_path=False, w_ctx=None):
    ...
    if not _valid_fname(fname):
        space.ec.warn("readfile() expects parameter 1 "
                      "to be a valid path, string given")
        return space.w_False
    ...
    try:
        ...
        f.close()
        return space.newint(len(res))
    except OSError, e:
        space.ec.warn("readfile(%s): failed to open stream: "
                      "%s" % (fname, os.strerror(e.errno)))
        return space.w_False
    except IOError, e:
        if not we_are_translated():
            space.ec.warn("readfile(%s): failed to open stream: "
...
def tempnam(space, dname, prefix):
    ...
    try:
        if os.path.isdir(dname):
            res = os.path.join(dname, prefix + tmpname)
        else:
            res = os.path.join("/tmp", prefix + tmpname)
    except TypeError:
        return space.w_False
...
def tmpfile(space):
    ...
    fd = os.tmpfile()
    w_res = W_FileResource(space, '<tmpfile>', 'w+')
    w_res.resource = fd
    w_res.state = 1
    return w_res
...
def is_in_basedir(space, func_name, path):
    ...
    if basedir:
        maxl = space.int_w(interp.constants["PHP_MAXPATHLEN"])
        if len(path) > maxl:
