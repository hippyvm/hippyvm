...
def _mktime(interp, func_name, is_gmt, hour, minute, second,
    ...
    if dst != -1:
        interp.space.ec.deprecated(
            "%s(): The is_dst parameter is deprecated" % func_name
        )
        if is_gmt:
            if dst == 1:
                adjust_seconds -= 3600
...
def mktime(interp, num_args, hour=0, minute=0, second=0,
    ...
    if not num_args:
        interp.space.ec.strict(
            "%s(): You should be using the time() function instead" % func_name)
        return interp.space.wrap(0)

    return interp.space.wrap(
        _mktime(interp, func_name, False, hour, minute, second, month, day, year, dst)
    )
...
def gmmktime(interp, num_args, hour=0, minute=0, second=0,
    ...
    return interp.space.wrap(
        _mktime(interp, func_name, True, hour, minute, second, month, day, year, dst)
    )
...
def _date_parse(space, timelib_time, errors):
    return_val = []
...
@wrap(['interp', str, str], error=False)
...
def date_offset_get(interp, w_date):
    return common.date_offset_get(interp, w_date)


@wrap(['interp',
       InstanceUnwrapper(W_DateTimeZone, 'DateTimeZone'),
       InstanceUnwrapper(W_DateTime, 'DateTime')], error=False)
def timezone_offset_get(interp, w_datetimezone, w_datetime):
    return common.timezone_offset_get(interp, w_datetimezone, w_datetime)


@wrap(['interp', InstanceUnwrapper(W_DateTimeZone, 'DateTimeZone')], error=False)
