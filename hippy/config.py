...
class Config(object):
    ...
    def __init__(self, space):
        ...
        self.ini = {
            ...
            'mysql.default_host': space.wrap('locahost'),
            'mysql.max_links': space.wrap(-1),
            'mysql.max_persistent': space.wrap(-1),
            'sql.safe_mode': space.wrap(0),
            'mysql_default_user': space.wrap('root'),
            'mysql_default_password': space.wrap('root'),
            ...
            'register_argc_argv': space.wrap(0),
    ...
    def token(self):
        ...
        for rule, token_type in self.rules:
            ...
            if m:
                ...
                tok = Token(token_type, val, self.lineno)
...
class IniReader(BaseParser):
    ...
    def var_string_list_tc_constant(self, p):
        w_const = self.interp.constants.get(p[0].getstr(), None)
        if w_const is None:
