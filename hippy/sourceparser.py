...
from hippy.lexer import PRECEDENCES, Lexer, ALL_RULES, BaseLexer
...
from hippy.ast import (
    ...
    DirMagic, ClassMagic, MethodMagic, FunctionMagic, ObjectDimList,
    PropertyDecl, MethodDecl, ParseError, BackTick, InstanceOf)
...
class LexerWrapper(BaseLexer):
    ...
    def next(self):
        while True:
            ...
            if tok is None or tok.name not in ('H_NEW_LINE', 'H_WHITESPACE',
...
class SourceParser(BaseParser):
    ...
    def __init__(self, space, lexer, filename=None):
        self.space = space
        self.lexer = lexer
    ...
    def additional_catch_def(self, p):
        fqcn = p[2]
        assert isinstance(fqcn, ConstantStr)
        return CatchBlock(fqcn.strval, p[3].getstr()[1:], p[6],
    ...
    def backticks_expr_t_encapsed(self, p):
        return ConstantStr(p[1].getstr(), lineno=p[1].lineno)
    ...
    def expr_oper_expr_instanceof(self, p):
        ...
        return InstanceOf(p[0],
            GetClass(right, lineno=p[2].getsourcepos(), complain=False),
    ...
    def compound_variable_t_variable(self, p):
        lineno = p[0].getsourcepos()
        return NamedVariable(p[0].getstr()[1:], lineno=lineno)
    ...
    def function_call_namespace(self, p):
        return SimpleCall(ConstantStr(p[0].getstr()), p[2].getstmtlist(),
                          lineno=p[0].getsourcepos())
    ...
    def _new_argument(self, opt_class_type, is_reference, t_variable,
        ...
        if opt_class_type is not None:
            assert isinstance(opt_class_type, ConstantStr)
            typehint = opt_class_type.strval
        else:
            typehint = None
    ...
    def non_empty_array_pair_list_list_ref_w_variable(self, p):
        hash = p[0]
        assert isinstance(hash, Hash)
        ref = Reference(p[3], lineno=p[0].getsourcepos())
    ...
    def unticked_cls_decl_stmt_class_entry_type(self, p):
        ...
        if x is None:
            ...
            assert isinstance(x, ConstantStr)
            extends = x.strval
        return ClassBlock(p[1].getstr(), c.intval, extends=extends,
                         baseinterfaces=p[3].unwrap_as_strings(), body=p[5],
                         lineno=p[1].getsourcepos())
    ...
    def unticked_cls_decl_stmt_interface_entry(self, p):
        return ClassBlock(p[1].getstr(),
                         ...
                         baseinterfaces=p[2].unwrap_as_strings(), body=p[4],
                         lineno=p[1].getsourcepos())
    ...
    def class_statement_method(self, p):
        ...
        decl = MethodDecl(p[3].getstr(), p[2] is not None,
    ...
    def class_name_static(self, p):
        return ConstantStr('static', lineno=p[0].getsourcepos())
    ...
    def method_or_not_method(self, p):
        return ObjectDimList(p[0], [], lineno=p[0].getsourcepos())
    ...
    def static_member_class_name(self, p):
        ...
        if isinstance(v, Variable):
            return StaticMember(p[0], v.node, lineno=p[1].getsourcepos())
            ...
            parent_node.node = StaticMember(p[0], node.node,
                                            lineno=p[1].getsourcepos())
            return v
        else:
            raise ParseError("Unexpected T_PAAMAYIM_NEKUDOTAYIM",
                             p[1].getsourcepos())
    ...
    def error_handler(self, token):
        raise ParseError("syntax error, unexpected \'%s\', file %s" %
                         (token.gettokentype(), self.filename),
                         token.getsourcepos())
...
def get_lexer(are_we_translated):
    return Lexer(use_rsre=are_we_translated)


def parse(space, _source, startlineno):
    lx = get_lexer(we_are_translated())
    lx.input(_source + ';', 0, startlineno)
    parser = SourceParser(space, lx)
    return parser.parse()
